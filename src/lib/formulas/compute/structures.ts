...
export function loadTotal(v: FormulaValues): FormulaResult {
...
export function loadCombination(v: FormulaValues): FormulaResult {
  const g = parseNum(v.permanent)
  const q = parseNum(v.exploitation)
...
export function linearLoad(v: FormulaValues): FormulaResult {
...
export function kpaToLinear(v: FormulaValues): FormulaResult {
...
export function convertForce(v: FormulaValues): FormulaResult {
...
export function convertPressure(v: FormulaValues): FormulaResult {
  const val = parseNum(v.value)
  if (val == null) return { error: 'Valeur requise.' }
...
export function beamMoment(v: FormulaValues): FormulaResult {
...
export function beamShear(v: FormulaValues): FormulaResult {
  const q = parseNum(v.load)
  const L = parseNum(v.span)
  if (q == null || L == null) return { error: 'Charge et portée requises.' }
  const V = (q * L) / 2
...
export function beamReaction(v: FormulaValues): FormulaResult {
  const q = parseNum(v.load)
  const L = parseNum(v.span)
  if (q == null || L == null) return { error: 'Charge et portée requises.' }
  const R = (q * L) / 2
...
export function normalStress(v: FormulaValues): FormulaResult {
...
export function shearStress(v: FormulaValues): FormulaResult {
...
export function simpleDeflection(v: FormulaValues): FormulaResult {
  const q = parseNum(v.load)
  const L = parseNum(v.span)
...
export function rectArea(v: FormulaValues): FormulaResult {
...
export function tributaryArea(v: FormulaValues): FormulaResult {
  return rectArea(v)
}

export function volumeRect(v: FormulaValues): FormulaResult {
  const l = parseNum(v.length)
  const w = parseNum(v.width)
...
export function weightVolume(v: FormulaValues): FormulaResult {
...
export function pointLoadMoment(v: FormulaValues): FormulaResult {
...
export function distributedLoad(v: FormulaValues): FormulaResult {
  const q = parseNum(v.load)
  const L = parseNum(v.span)
