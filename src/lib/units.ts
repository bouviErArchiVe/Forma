...
export const UNITS_BY_CATEGORY: Record<UnitCategoryId, UnitDef[]> = {
  length: [
...
export function parseScaleFactor(scaleStr: string): number {
  ...
  if (eq) {
    ...
    if (drawIn != null && realIn != null && drawIn > 0 && realIn > 0) return realIn / drawIn
  }
  return 50
}

function parseImperialMeasure(str: string): number | null {
...
export function convertDrawingScale(
  value: string | number,
  fromUnitId: string,
  scaleStr: string,
): { mm: number; cm: number; m: number } | null {
