...
const MOCK = [
  {id:"1",title:"Maison Unifamiliale — Projet 1",subject:"arch",   template:"plan",   pages:18,starred:true, modified:"Aujourd'hui"},
  ...
  {id:"4",title:"Mathématiques Avancées",         subject:"math",   template:"math",   pages:22,starred:true, modified:"18 mai"},
]
...
export default function LibraryPage() {
  ...
  const toggleStar = (id,e) => { e.stopPropagation(); setNotebooks(ns=>ns.map(n=>n.id===id?{...n,starred:!n.starred}:n)) }
  const create = () => {
    if(!newTitle.trim()) return
    const nb={id:Date.now().toString(),title:newTitle,subject:newSubj,template:newTmpl,pages:1,starred:false,modified:"Maintenant"}
    setNotebooks(p=>[nb,...p]); setShowNew(false); setNewTitle(""); open(nb)
  }
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div style={{background:T.surface,borderRadius:20,padding:28,width:520,maxWidth:"94vw",maxHeight:"88vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.25)"}}>
            ...
            <div style={{marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:T.muted,marginBottom:6,letterSpacing:.5}}>TITRE</div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:T.muted,marginBottom:6,letterSpacing:.5}}>MATIÈRE</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,maxHeight:200,overflowY:"auto"}}>
                {SUBJECTS.map(s=>(
                  <button key={s.id} onClick={()=>setNewSubj(s.id)}
                    style={{padding:"8px 5px",borderRadius:9,border:`1px solid ${newSubj===s.id?s.c:T.border}`,background:newSubj===s.id?s.c+"18":T.bg,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,transition:"all .12s"}}>
            ...
            <div style={{marginBottom:22}}>
              <div style={{fontSize:11,fontWeight:700,color:T.muted,marginBottom:6,letterSpacing:.5}}>MODÈLE DE PAGE</div>
            ...
            <button onClick={create} disabled={!newTitle.trim()}
              style={{width:"100%",padding:13,borderRadius:11,background:newTitle.trim()?T.accent:T.border,border:"none",color:"#fff",fontWeight:700,fontSize:15,cursor:newTitle.trim()?"pointer":"not-allowed",transition:"all .2s"}}>
              Créer le carnet →
            </button>
          </div>
        </div>
      ...
      <div style={{background:T.surface,borderBottom:`1px solid ${T.border}`,padding:"0 24px",position:"sticky",top:0,zIndex:20,boxShadow:"0 1px 12px rgba(0,0,0,.06)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
          ...
          <button onClick={()=>setShowNew(true)}
            style={{padding:"8px 16px",borderRadius:9,background:T.accent,border:"none",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",boxShadow:`0 3px 12px ${T.accent}44`,transition:"all .2s"}}
      ...
      <div style={{padding:"20px 24px",maxWidth:1400,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:22}}>
          {[
            ...
            {l:"Pages",    v:notebooks.reduce((a,n)=>a+n.pages,0),    c:T.a3,      e:"📄"},
          ].map(s=>(
            <div key={s.l} style={{padding:"16px 20px",borderRadius:14,background:T.surface,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:14,cursor:"default",transition:"all .2s"}}
        ...
        <div style={{display:"flex",gap:7,marginBottom:14,flexWrap:"wrap"}}>
          ...
          {usedSubjects.map(s=>(
            <button key={s.id} onClick={()=>setFilt(s.id)} style={{padding:"6px 13px",borderRadius:20,border:`1px solid ${filt===s.id?s.c:T.border}`,background:filt===s.id?s.c+"18":T.bg,color:filt===s.id?s.c:T.muted,fontSize:12,cursor:"pointer",transition:"all .15s"}}>
              {s.e} {s.l}
            </button>
          ))}
        </div>
        ...
        <div style={{position:"relative",marginBottom:22}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher dans vos carnets…"
            ...
            return(
              <div key={nb.id} onClick={()=>open(nb)}
                ...
                <div style={{height:96,background:`linear-gradient(135deg,${s.c}22,${s.c}06)`,display:"flex",alignItems:"center",justifyContent:"center",borderBottom:`1px solid ${T.border}`,position:"relative"}}>
                  ...
                  <div style={{position:"absolute",top:7,right:7,fontSize:10,color:s.c+"88",fontFamily:"monospace"}}>{t.i}</div>
                </div>
                <div style={{padding:"11px 14px"}}>
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:5}}>
                    ...
                    <button onClick={e=>toggleStar(nb.id,e)} style={{background:"none",border:"none",color:nb.starred?"#f5a623":T.border,cursor:"pointer",fontSize:14,flexShrink:0}}>★</button>
                  </div>
                  <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                    <div style={{padding:"2px 7px",borderRadius:20,background:s.c+"18",color:s.c,fontSize:9,fontWeight:700}}>{s.l}</div>
                    <div style={{fontSize:9,color:T.muted,fontFamily:"monospace"}}>{nb.pages}p</div>
                    <div style={{fontSize:9,color:T.muted}}>{nb.modified}</div>
                  </div>
                </div>
              </div>
            )
        ...
        {filtered.length===0&&<div style={{textAlign:"center",padding:"60px 0",color:T.muted}}><div style={{fontSize:36,marginBottom:10}}>🔍</div><div>Aucun carnet trouvé</div></div>}
      </div>
    </div>
  )
}
