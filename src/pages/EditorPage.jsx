...
/* ── CANVAS ─────────────────────────────────────────────────── */
function DrawCanvas({tool,color,size,cRef}){
  const drawing=useRef(false),strokes=useRef([]),cur=useRef([])
  const redraw=useCallback(()=>{const c=cRef.current;if(!c)return;const ctx=c.getContext("2d");ctx.clearRect(0,0,794,1123);strokes.current.forEach(s=>{if(s.pts.length<2)return;ctx.beginPath();ctx.strokeStyle=s.color;ctx.lineWidth=s.size;ctx.lineCap="round";ctx.lineJoin="round";ctx.globalAlpha=s.tool==="highlight"?.4:1;ctx.globalCompositeOperation=s.tool==="eraser"?"destination-out":"source-over";ctx.moveTo(s.pts[0].x,s.pts[0].y);s.pts.forEach(p=>ctx.lineTo(p.x,p.y));ctx.stroke()});ctx.globalCompositeOperation="source-over";ctx.globalAlpha=1},[cRef])
  ...
  const mv=e=>{if(!drawing.current)return;e.preventDefault();const p=gP(e);cur.current.push(p);const c=cRef.current;const ctx=c.getContext("2d");const pts=cur.current;if(pts.length<2)return;ctx.beginPath();ctx.strokeStyle=color;ctx.lineWidth=size;ctx.lineCap="round";ctx.lineJoin="round";ctx.globalAlpha=tool==="highlight"?.4:1;ctx.globalCompositeOperation=tool==="eraser"?"destination-out":"source-over";ctx.moveTo(pts[pts.length-2].x,pts[pts.length-2].y);ctx.lineTo(pts[pts.length-1].x,pts[pts.length-1].y);ctx.stroke();ctx.globalCompositeOperation="source-over";ctx.globalAlpha=1}
  const up=()=>{if(!drawing.current)return;drawing.current=false;strokes.current.push({pts:[...cur.current],color,size,tool});cur.current=[]}
  useEffect(()=>{window.__undo=()=>{strokes.current.pop();redraw()};window.__clear=()=>{strokes.current=[];redraw()}},[redraw])
  return<canvas ref={cRef}width={794}height={1123}style={{position:"absolute",inset:0,width:"100%",height:"100%",cursor:tool==="eraser"?"cell":"crosshair",touchAction:"none",zIndex:5}}onMouseDown={dn}onMouseMove={mv}onMouseUp={up}onMouseLeave={up}onTouchStart={dn}onTouchMove={mv}onTouchEnd={up}/>
}
...
export default function EditorPage() {
  ...
  const nb = activeNotebook || {id:"1",title:"Carnet",subject:"arch",template:"plan",pages:1}
  const cRef = useRef()
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100vh",background:T.bg,fontFamily:"'Nunito',sans-serif",overflow:"hidden",color:T.ink}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
              {THEMES_LIST.map(th=>(
                <button key={th.id} onClick={()=>{changeTheme(th);setShowTheme(false)}}
                  ...
                  <div style={{padding:"6px 10px",background:th.bg,textAlign:"left"}}>
        ...
        <div style={{width:56,background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",alignItems:"center",padding:"8px 0",gap:2,flexShrink:0,overflowY:"auto"}}>
          <div style={{display:"flex",width:"100%",borderBottom:`1px solid ${T.border}`,marginBottom:4,paddingBottom:4}}>
            {[["tools","🔧"],["clr","🎨"],["brush","✏"]].map(([t,ic])=>(
              <button key={t} onClick={()=>setLeftTab(t)} style={{flex:1,height:28,background:leftTab===t?`${T.accent}18`:"transparent",border:"none",cursor:"pointer",fontSize:11,color:leftTab===t?T.accent:T.muted}}>
                {ic}
              </button>
            ))}
          </div>
        ...
        <div style={{flex:1,overflow:"auto",background:T.bg,display:"flex",justifyContent:"center",alignItems:"flex-start",padding:"20px"}} id="canvas-area">
          <div style={{transform:`scale(${zoom})`,transformOrigin:"top center",transition:"transform .12s",position:"relative",flexShrink:0}}>
            <div style={{width:794,height:1123,position:"relative",boxShadow:"0 4px 40px rgba(0,0,0,.2)"}}>
              ...
              <DrawCanvas tool={tool} color={activeColor} size={sz} cRef={cRef}/>
            </div>
          </div>
        </div>
      ...
      <div style={{height:38,background:T.surface,borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",padding:"0 16px",gap:14,zIndex:20}}>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          ...
          <span style={{fontSize:10,color:T.muted,fontFamily:"monospace"}}>{page.toString().padStart(2,"0")} / {(nb.pages||1).toString().padStart(2,"0")}</span>
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5}}>
          ...
          <span style={{fontSize:9,color:T.muted}}>Auto-sauvegarde</span>
        </div>
      </div>
    </div>
  )
