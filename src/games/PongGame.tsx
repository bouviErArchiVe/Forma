...
export default function PongGame({ T, onClose, bestScore = 0, onSaveScore }: GameProps) {
  ...
  useEffect(() => {
    ...
    const movePaddle = (clientY: number) => {
      ...
      s.p1 = Math.max(8, Math.min(H - 88, y))
    }

    const onMove = (e: MouseEvent) => movePaddle(e.clientY)
  ...
  return (
    <GameShell
      T={T}
      title="Pong"
      hint="Souris ou doigt sur la raquette gauche"
      score={score}
      best={bestScore}
      onClose={onClose}
    >
