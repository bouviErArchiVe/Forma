...
export default function BreakoutGame({ T, onClose, bestScore = 0, onSaveScore }: GameProps) {
  ...
  useEffect(() => {
    ...
    const move = (clientX: number) => {
      ...
      s.paddle = Math.max(0, Math.min(W - 90, ((clientX - rect.left) / rect.width) * W - 45))
    }
    ...
    const onRestart = (e: KeyboardEvent) => {
      if (e.code === 'Enter' && stateRef.current && !stateRef.current.alive) reset()
    }
  ...
  return (
    <GameShell
      T={T}
      title="Breakout"
      hint="Souris, toucher ou flèches"
      score={score}
      best={bestScore}
      onClose={onClose}
    >
