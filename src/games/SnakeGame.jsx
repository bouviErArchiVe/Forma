...
export default function SnakeGame({ T, onClose, bestScore = 0, onSaveScore }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (DIR[e.code]) { e.preventDefault(); setDir(DIR[e.code]) } }
  ...
  useEffect(() => {
    const onKey = (e) => { if (over && e.code === 'Enter') reset() }
    window.addEventListener('keydown', onKey)
