 ...
 * Local-first, versions, cloud optionnel via storageKey.
 */
export function useModuleSync({
  ...
  }, [enabled, resourceId, storageKey, getPayload, versionSnapshots, setLastLocalSaveAt, setGlobalStatus, resourceType])
  ...
  useEffect(() => {
    const flush = () => { if (timer.current) { clearTimeout(timer.current); saveNow() } }
